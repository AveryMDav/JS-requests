...
sayHelloButton.addEventListener('mouseover', changeStyle)
...
const repeatMyParam = () => {
    ...
    .then(function(res){
        ...
        let str = res.data
        document.querySelector('#repeat-text').textContent = str
        let strStl = document.querySelector('#repeat-text')
        strStl.style.display = 'block'
    })
    .catch(function(err){console.log(err)})
}
...
function queryTest(){
    axios
    .get(`${baseURL}/query-test?avery=student`)
    .then(function(res){
        console.log(res.data)
    })
    .catch(function(err){console.log(err)})
